...
function getDependencies(depMap) {
    ...
    depMap.forEach(dep => {
        ...
        const ranges = getPomVersionFormat(dep.versionRanges.map(r => getPomRange(r, dep.resolvedVersion)), dep.resolvedVersion);
        ranges.forEach(r => {
            resp.push('<dependency>\n' +
...
function getGroupAndArt(name) {
    ...
    if (parts.length > 1) {
        return {
            group: parts.splice(0, parts.length - 1).join(Constants.GROUP_SEPARATOR).replace('@', ''),
